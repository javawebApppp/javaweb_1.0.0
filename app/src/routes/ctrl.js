...
const uploadFile = async (req, res) => {
  if (req.method === "POST") {
    ...
    await uploadMiddleware(req, res, async (err) => {
      if (err) {
        ...
        return res
          .status(400)
          .send(
            '<script>alert("datafile이 없습니다. 파일을 선택해주세요."); window.location.href="/";</script>'
          );
      } else {
        ...
        if (!coreNum || !taskNum) {
          return res
            .status(400)
            .send(
              '<script>alert("coreNum 또는 taskNum이 없습니다. 값을 입력해주세요."); window.location.href="/";</script>'
            );
        }

        const data = await fs.promises.readFile(
          "./app/uploads/inputFile.txt",
          "utf8"
        );
    ...
    const updatedInputNumbers = require("../../inputnumber");
    res.render("./index", { data: updatedInputNumbers });
  }
};
...
const updateInputNumbers = async (newCorenum, newTasknum) => {
  ...
  module.exports = inputNumbers;\n`;
    ...
    await fs.promises.writeFile("./app/inputnumber.js", content, "utf8");
  } catch (e) {
